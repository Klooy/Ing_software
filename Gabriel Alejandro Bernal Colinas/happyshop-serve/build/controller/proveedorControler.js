...
class ProveedorController {
    listar(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const consulta = yield database_1.default.query('SELECT * FROM Proveedor', (error, result) => {
                if (error)
                    throw error;
                if (result.length === 0) {
                    res.json({ message: "La Proveedor no existe" });
                }
                else {
                    res.json(result);
                }
    ...
    obtener(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const resul = yield database_1.default.query('SELECT * FROM Proveedor WHERE idProveedor = ?', [id], (error, result) => {
                if (error)
                    throw error;
                if (result.length === 0) {
                    res.json({ message: "La Proveedor no existe" });
    ...
    delete(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            res.json({ menssage: 'Proveedor eliminada' });
        });
    }
}
