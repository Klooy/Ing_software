...
export class CuentaComponent implements OnInit {
  ...
  constructor(private activarMiCuenta: ActivarMiCuentaService, private personaService: PersonaService, private clienteService: ClientesService,
    private carritoService: CarritoService, private router: Router, private paisService: PaisService, private ciudadService: CiudadService,
    private actiEditService: ActiEditDirecService, private detallePedidoService: DetallePedidoService, private pedidoService: PedidoService) { }
  ...
  eliminarCarrito(){
    this.carritoService.eliminarTodo(this.idCliente).subscribe(
      res => {
        console.log('vaciar carrito', res);
  ...
  obtenerPedido(){
    let datos: any;
    this.pedidoService.obtenerUnico(this.cliente.idCliente).subscribe(
      res => {
        datos = res;

        for(let i = 0; i < datos.length; i++){
          this.pedidoDB = datos[i];
  ...
  obtenerDetallePedido(){
    this.detallePedidoService.obtenerUnico(this.pedidoDB.idPedido).subscribe(
      res => {
